		...
		if (global.db.groups && global.db.groups[id] && store.groupMetadata && store.groupMetadata[id]) {
			...
			for (let n of participants) {
				...
				try {
					...
					profile = 'https://ar-hosting.pages.dev/1739562361203.jpg';
				}
				let messageText;
				if (action === 'add') {
					...
					messageText = `@${n.split('@')[0]} telah menjadi Admin`;
					updateAdminStatus(participants, metadata.participants, 'admin');
				} else if (action === 'demote') {
					messageText = `@${n.split('@')[0]} berhenti sebagai Admin`;
				...
				if (messageText && global.db.groups[id].welcome) {
					await naze.sendMessage(id, {
						...
						contextInfo: {
							...
							externalAdReply: {
								title: action == 'add' ? 'Welcome👋' : action == 'remove' ? 'Good Bye👋' : action.charAt(0).toUpperCase() + action.slice(1),
								mediaType: 1,
								previewType: 0,
...
async function LoadDataBase(naze, m) {
	try {
		...
		if (setBot) {
			...
			if (!('readsw' in setBot)) setBot.readsw = true
...
async function MessagesUpsert(naze, message, store, groupCache) {
	try {
		...
		require('../naze')(naze, m, message, store, groupCache);
